    ...
    className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 my-10 gap-5 dark:text-black'>
        {workData.map((project, index)=>(
            ...
            style={{ backgroundImage: `url(${project.bgImage.src})` }}>
                <div className='bg-[#F0AD4E] w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex items-center justify-between duration-500 group-hover:bottom-7'>
                    <div>
                        <h2 className='font-semibold'>{project.title}</h2>
                        <p className='text-sm text-gray-700'>{project.description}</p>
                    </div>
                    <div className='bg-white border rounded-full border-black w-9 aspect-video flex items-center justify-center shadow-[2px_2px_0_#000] group-hover:bg-orange-300 transition'>
